...
-- First, let's see the current state and expected pattern
DO $$
DECLARE
    ...
    seat_count BIGINT;
    expected_seats BIGINT;
BEGIN
    RAISE NOTICE '=== SEAT ID NUMBERING AUDIT ===';
    
    FOR current_show_id IN 1..122 LOOP
        -- Calculate expected seat ID range for this show
        SELECT (current_show_id - 1) * 120 + 1, current_show_id * 120 INTO expected_min_id, expected_max_id;
        ...
        -- Get expected seat count from screen
        SELECT sc.total_rows * sc.seats_per_row INTO expected_seats
        FROM shows s
        JOIN screens sc ON s.screen_id = sc.id
        WHERE s.id = current_show_id;
        
        IF seat_count > 0 THEN
            IF actual_min_id != expected_min_id OR actual_max_id != expected_max_id THEN
                RAISE NOTICE 'Show %: Expected %-%, Actual %-% (Count: %)', 
                    current_show_id, expected_min_id, expected_max_id, actual_min_id, actual_max_id, seat_count;
            END IF;
        ELSE
            RAISE NOTICE 'Show %: NO SEATS FOUND', current_show_id;
        END IF;
    END LOOP;
...
-- Now fix the seat numbering by recreating show_seats with correct IDs
-- This is a comprehensive fix that ensures every show has the right seat IDs
...
SELECT 
    ...
    (s.id - 1) * 120 + (st.row_number - 1) * st.seats_per_row + st.seat_number as id,
    s.id as show_id,
    st.row_number,
    st.seat_number,
    st.category,
